...
from tdeptools.geometry import compute_area, compute_mean_distance_to_centroid
import collections

echo = rich.print

_outfile_plot = "fc3_TAGs.pdf"
...
_cls_data = collections.namedtuple(
    "FC3_block_data",
    ["index_triplet", "i1", "i2", "i3", "L1", "L2", "L3", "force_constant"],
)
_cls_block = collections.namedtuple("FC3_block", ["atom_index", "n_neighbors", "data"])
...
def get_one_block(f):
    ...
    for itriplet in range(n_neighbors):
        ...
        data.append(_cls_data(itriplet, i1, i2, i3, L1, L2, L3, MM))
...
def process_blocks(blocks: list, atoms: Atoms = None, decimals: int = 10):
    # sort data
    triplet_indices = []  # indices in unit cell
    triplet_numbers = []  # element numbers
    triplet_symbols = []  # element symbols

    triplet_distance_12 = []  # distance between atom 1 and 2
    triplet_distance_13 = []  # distance between atom 1 and 3
    triplet_distance_23 = []  # distance between atom 2 and 3

    triplet_areas = []
    triplet_mean_distances = []
    ...
    for block in blocks:
        ...
        for triplet in data:
            itriplet, i1, i2, i3, L1, L2, L3, MM = triplet
            ...
            dr13 = r1 - r3
            dr23 = r2 - r3
            r12 = np.linalg.norm(dr12)
            r13 = np.linalg.norm(dr13)
            r23 = np.linalg.norm(dr23)

            triplet_areas.append(compute_area(r1, r2, r3))
            mean_distance = compute_mean_distance_to_centroid([r1, r2, r3])
            triplet_mean_distances.append(mean_distance)

            triplet_distance_12.append(r12)
            triplet_distance_13.append(r13)
            triplet_distance_23.append(r23)
            triplet_indices.append([iatom, i1, i2, i3])
            n1, n2, n3 = (
                atoms.numbers[i1 - 1],
                atoms.numbers[i2 - 1],
                atoms.numbers[i3 - 1],
            ...
            triplet_numbers.append(numbers)
            symbols = "-".join([chemical_symbols[ii] for ii in numbers])
            triplet_symbols.append(symbols)
    ...
    triplet_numbers = np.array(triplet_numbers)
    triplet_symbols = np.array(triplet_symbols)
    # distance_norms = np.array(distance_norms)
    triplet_distance_12 = np.array(triplet_distance_12)
    triplet_distance_13 = np.array(triplet_distance_13)
    triplet_distance_23 = np.array(triplet_distance_23)
    triplet_areas = np.array(triplet_areas)
    triplet_mean_distances = np.array(triplet_mean_distances)
    ...
    r_shell_12, i_shell = np.unique(
        triplet_mean_distances.round(decimals=10), return_inverse=True
    )

    data = {
        "r12": triplet_distance_12,
        "r13": triplet_distance_13,
        "r23": triplet_distance_23,
        "shell_index": i_shell,
        "area": triplet_areas,
        "mean_distance": triplet_mean_distances,
        "fc_norm": forceconstant_norms,
        "fc_trace": forceconstant_traces,
        "symbols": triplet_symbols,
        "i1": triplet_indices[:, 0],
        "i2": triplet_indices[:, 1],
        "i3": triplet_indices[:, 2],
    }
...
@app.command()
def main(
    file: Path = "outfile.forceconstant_thirdorder",
    file_primitive: Path = "infile.ucposcar",
    outfile_data: Path = "fc3_norms_and_traces.csv",
    plot: bool = True,
    log: bool = False,
    trace: bool = False,
    ...
    if not selfterms:
        ...
        df = df[df[["r12", "r13", "r23"]].gt(0).all(axis=1)]
